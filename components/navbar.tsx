...
export function Navbar({ role }: { role: Role | null }) {
  ...
  const links: Array<{ href: string; label: string; icon: typeof Home; roles?: Role[] }> = [
    ...
    { href: '/match', label: 'AI Match', icon: Sparkles, roles: ['STUDENT'] },
    ...
    <header className="sticky top-0 z-40 h-16 border-b border-slate-200/50 bg-white/80 backdrop-blur-xl shadow-soft">
      <div className="mx-auto flex h-full max-w-[1280px] items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          <SignedIn>
            <Link href="/settings/profile" prefetch={shouldPrefetch} className="hidden h-10 w-10 place-items-center rounded-[10px] border border-slate-200/50 bg-white/80 backdrop-blur-sm text-slate-500 transition-all duration-300 hover:text-primary-500 hover:shadow-soft md:grid" aria-label="Settings">
